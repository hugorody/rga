...
################################################################################
# Inputs
coilsfile = "/home/hugo/Dropbox/Esalq/Sunflower/Ha412-Eugene/predicted_proteins_HAN412.fasta.coils" #coils predictions output
tmhmmfile = "/home/hugo/Dropbox/Esalq/Sunflower/Ha412-Eugene/predicted_proteins_HAN412.fasta.tmhmm2" #tmhmm predictions output
interprofile = "/home/hugo/Dropbox/Esalq/Sunflower/Ha412-Eugene/predicted_proteins_HAN412.fasta.tsv" #interpro predictions output
pfamfile = "/home/hugo/Dropbox/Esalq/Sunflower/Ha412-Eugene/predicted_proteins_HAN412.fasta.pfamscan" #pfam_scan predctions output
phobiusfile = "/home/hugo/Dropbox/Esalq/Sunflower/Ha412-Eugene/predicted_proteins_HAN412.fasta.phobius" #phobius predictions output
targetpfile = "/home/hugo/Dropbox/Esalq/Sunflower/Ha412-Eugene/predicted_proteins_HAN412.gff3" #targetp2 predictions short output
fastafile = "/home/hugo/Dropbox/Esalq/Sunflower/Ha412-Eugene/predicted_proteins_HAN412.fasta" #fasta file

#outputs
out_trueRGAs = open("RGAdb_2020.tsv","w") #sequences harboring RGA basic domains
out_domains = open("RGAdb_2020domains.txt","w")
################################################################################
#Parse FASTA protein sequences used for predictions

fasta = {}
with open(fastafile,"r") as set1:
    for i in SeqIO.parse(set1, "fasta"):
        seqID = str(i.id)
        fasta[seqID] = str(i.seq)
...
with open(targetpfile,"r") as set1:
    for i in set1:
        ...
        if "#" not in i[0]:
            geneID = i[0]
            signal = i[-1].replace("Note=","")
            if signal == ".":
                signal = "noTP"
            start = i[3]
            stop = i[4]
            targetp2DBfeatures[geneID] = [["TargetP2",signal,start,stop]]

print ("Sequences with domains predicted by TargetP2:",len([i for i in targetp2DBfeatures.items() if i[1][0][1] != "noTP"]))
...
df1 = pd.read_csv(interprofile,sep="\t",error_bad_lines=False)
...
TARGETP2 = addDOMAINS(targetp2DBfeatures)
...
for i in newtrueRGAs.items():
    if i[0] in fasta: #had to add this because predictions were made for all ORFS > 30
